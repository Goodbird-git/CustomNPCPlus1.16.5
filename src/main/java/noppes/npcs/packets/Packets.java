...
public class Packets
{
    ...
    public static void register() {
        Packets.Channel = NetworkRegistry.ChannelBuilder.named(new ResourceLocation("customnpcs", "packets")).clientAcceptedVersions((Predicate)"CNPCS"::equals).serverAcceptedVersions((Predicate)"CNPCS"::equals).networkProtocolVersion(() -> "CNPCS").simpleChannel();
        Packets.index = 0;
        ...
        Packets.Channel.registerMessage(Packets.index++, PacketPlayerSkinGet.class, PacketPlayerSkinGet::encode, PacketPlayerSkinGet::decode, PacketBasic::handle);
        Packets.Channel.registerMessage(Packets.index++, PacketPlayerSkinAdd.class, PacketPlayerSkinAdd::encode, PacketPlayerSkinAdd::decode, PacketBasic::handle);
        ...
        Packets.Channel.registerMessage(Packets.index++, SPacketPlayerSkinSet.class, SPacketPlayerSkinSet::encode, SPacketPlayerSkinSet::decode, PacketServerBasic::handle);
    ...
    public static <MSG> void sendNearby(final World level, final BlockPos pos, final int range, final MSG msg) {
        Packets.Channel.send(PacketDistributor.NEAR.with(() -> new PacketDistributor.TargetPoint((double)pos.getX(), (double)pos.getY(), (double)pos.getZ(), (double)range, level.dimension)), (Object)msg);
    }
    ...
    public static void doExplosion(final World level, final Explosion explosion, final float size) {
        ...
        for (final Object ob : level.players()) {
            ...
            final SExplosionPacket packet = new SExplosionPacket(vec.x, vec.y, vec.z, size, explosion.getToBlow(), (Vector3d)explosion.getHitPlayers().get(player));
